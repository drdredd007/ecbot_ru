...
export abstract class Roadmap {
    ...
    private static generateDevBreakdown(deliverables: any[any], compareTime: number, scheduledDeliverables: any[any], publish: boolean = false): { breakdown: string; deliverableRanks: any; } {
        ...
        deliverableTimes.forEach(dt => {
            ...
            deliverableRanks.push({
                deliverable_id: dt[0].deliverable_id, time: Math.round(GeneralHelpers.convertMillisecondsToDays(time) / 3), totalMembers: totalMembers,
        ...
        return {
            breakdown: GeneralHelpers.shortenText(`There are approximately ${adjustedTotalDevs} devs (out of ~${this.HiredDevs}, or ${Math.round(adjustedTotalDevs / this.HiredDevs * 100)}%) with ${totalDevs} assignments scheduled to work on ${deliverableTimes.length} observable deliverables. ` + // no way to determine unique developers
     ...
     */
    private static async exportJson(channel: MessagingChannel, discord: boolean = true) {
